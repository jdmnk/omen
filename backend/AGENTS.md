...
## General Rules (apply across repos)

- Keep changes simple and consistent; skip excessive edge-case handling unless required, and avoid adding new documentation markdown files unless asked.
- For UI work, favor clean professional styling without emojis or heavy color coding; prefer Shadcn components, use `cn` when combining Tailwind classes, and keep design choices aligned with the existing patterns (e.g., don't double-code YES/NO and BUY/SELL colors in the same element).
- For Next.js/React tasks, prefer the App Router, Shadcn + Tailwind, reuse shared Tailwind styles, export components with `export function MyComponent() {}`, design responsively and for performance (especially graphics), and default to pnpm for installs; only use default exports when the framework requires it.
